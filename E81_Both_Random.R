...
r[] <- uniform2
Mat.S.H <- t(matrix(sample(as.vector(r)),nrow=nrow(r),ncol=ncol(r)))
...
write.csv(df.PropsM,"TS_E81_RANDOM_BOTH.csv",quote=F,row.names=F)
write.csv(Mat.S,"DIST_E81_RANDOM_BOTH.csv",quote=F,row.names=F)
